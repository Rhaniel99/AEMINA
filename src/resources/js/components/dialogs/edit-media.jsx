import {
  ...
  export default function EditMedia({ open, onOpenChange, content }) {
    const renderForm = () => {
      switch (content?.name) {
        case "Filme":
          return (
            <>
              <Movie onSuccess={() => onOpenChange(false)} Movie={content}  />
            </>
          );
        case "Série":
          return (
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-[850px]" onPointerDownOutside={(event) => event.preventDefault()}>
          <DialogHeader>
            <DialogTitle>
              {content ? `Atualizar ${content.name}` : "Cadastro"}
            </DialogTitle>
            <DialogDescription>
              Preencha os campos abaixo para atualizar o conteúdo.
            </DialogDescription>
          </DialogHeader>
                {renderForm()}
        </DialogContent>
